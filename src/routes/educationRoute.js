...
router.get('/', async (req, res) => {
    try {
        let query = `SELECT * FROM my_schema.users_education;`
...
router.post('/:id', async (req, res) => {
    const { institution_name, degree, field, start_date, end_date, isPresent=false } = req.body
    ...
    try {
        let query = `INSERT INTO  my_schema.users_education (institution_name, degree, field, start_date, end_date, user_id)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *;`
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        let query = `UPDATE my_schema.users_education SET ${updateColumns} where id='${userId}'`
        console.log(query);
        let response = await client.query(query)
...
router.delete('/:id', async (req, res) => {
    const userId = req.params.id
    try {
        let query = `DELETE FROM my_schema.users_education where id='${userId}'`
        let response = await client.query(query)
        return res.status(201).json({ status: "success", message: "user experience delete successfully", result: response })
    } catch (error) {
        return res.status(500).json({ status: "failed", message: error.message })
